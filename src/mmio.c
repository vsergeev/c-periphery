...
int mmio_open_advanced(mmio_t *mmio, uintptr_t base, size_t size, const char *path) {
    ...
    if ((fd = open(path, O_RDWR | O_SYNC)) < 0)
        return _mmio_error(mmio, MMIO_ERROR_OPEN, errno, "Opening memory character device");
    ...
    if (close(fd) < 0) {
        ...
        mmio->ptr = 0;
        return _mmio_error(mmio, MMIO_ERROR_OPEN, errsv, "Closing memory character device");
    }
